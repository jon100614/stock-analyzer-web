...
# 初始化 session state
if "stock_query_run" not in st.session_state:
    st.session_state.stock_query_run = False


def trigger_stock_query():
    """觸發股票查詢"""
    st.session_state.stock_query_run = True
...
with st.sidebar:
    ...
    symbol = (
        st.text_input(
            "股票代碼",
            value="AAPL",
            help="例如：AAPL、TSLA、2330、2330.TW、3138.TWO、0700。輸入後按 Enter 或點查詢。",
            on_change=trigger_stock_query,
        )
        .upper()
        .strip()
    )
    ...
    st.button("查詢", type="primary", on_click=trigger_stock_query)

    st.divider()
    st.caption("輸入代碼後按 Enter 或點查詢，所有資訊會顯示在同一頁。")
...
# 點查詢或按 Enter 後執行
if st.session_state.stock_query_run:
    st.session_state.stock_query_run = False
    if not symbol:
        st.warning("請輸入股票代碼")
    else:
